...
def score(nice,mean,name):
    ...
    if nice > 2: #if condition is valid, call win function pasing in the variables so it can use them
        win(nice,mean,name)
...
def again(nice,mean,name):
    ...
    while stop:
        ...
        if choice == "y":
            playsound('zapsplat_household_doorbell_chime_classic_001_65580.mp3')
            stop = False
...
def reset(nice,mean,name):
    nice=0
    mean=0
    #notice, i do not rese the name variable as that same user has elected to play again
